...
def generate_ou_process(batch_size, num_dims, mu, tau, sigma, noise_std, dt = 0.1):
    ...
    for t in range(1, num_dims):
        dx = -(ou_x[:, t-1]-mu) / tau * dt + sigma * np.sqrt(2 / tau) * np.random.normal(0, 1, batch_size) * np.sqrt(dt)
        ou_x[:, t] =  ou_x[:, t-1] + dx

    ou_x_noise = ou_x + np.random.multivariate_normal(np.zeros(num_dims),
                                                      noise_std * np.eye(num_dims),
                                                      batch_size)
