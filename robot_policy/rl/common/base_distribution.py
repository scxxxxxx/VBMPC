...
class Distribution(ABC):
    ...
    @abstractmethod
    def distribution_net(self, *args, **kwargs) -> Union[nn.Module, Tuple[nn.Module, nn.Parameter]]:
    ...
    @abstractmethod
    def prob_distribution(self, *args, **kwargs) -> "Distribution":
    ...
    @abstractmethod
    def logp_from_params(self, *args, **kwargs) -> Tuple[torch.Tensor, torch.Tensor]:
        """
...
class DiagGaussianDistribution(Distribution):
    """
    Construct a Gaussian distribution with diagonal covariance matrix from the dimension of the action space,
    ...
    def distribution_net(self, latent_dim: int, log_std_init: float = 0.0) -> Tuple[nn.Module, nn.Parameter]:
        """
    ...
    def prob_distribution(self, mean_actions: torch.Tensor, log_std: torch.Tensor) -> "DiagGaussianDistribution":
        """
    ...
    def logp(self, actions: torch.Tensor) -> torch.Tensor:
        ...
        Note that you must first call the ``prob_distribution()`` method.
    ...
    def actions_from_params(self, mean_actions: torch.Tensor, log_std: torch.Tensor, deterministic: bool = False) -> torch.Tensor:
        ...
        self.distribution(mean_actions, log_std)
        return self.get_actions(deterministic=deterministic)

    def logp_from_params(self, mean_actions: torch.Tensor, log_std: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        """
...
class SquashedDiagGaussianDistribution(DiagGaussianDistribution):
    ...
    def prob_distribution(self, mean_actions: torch.Tensor, log_std: torch.Tensor) -> "SquashedDiagGaussianDistribution":
        super(SquashedDiagGaussianDistribution, self).distribution(mean_actions, log_std)
        return self

    def logp(self, actions: torch.Tensor, gaussian_actions: Optional[torch.Tensor] = None) -> torch.Tensor:
        # Inverse tanh
        ...
        log_prob = super(SquashedDiagGaussianDistribution, self).logp(gaussian_actions)
    ...
    def logp_from_params(self, mean_actions: torch.Tensor, log_std: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        log_prob = self.logp(action, self.gaussian_actions)
...
class CategoricalDistribution(Distribution):
    ...
    def distribution_net(self, latent_dim: int) -> nn.Module:
        """
    ...
    def prob_distribution(self, action_logits: torch.Tensor) -> "CategoricalDistribution":
    ...
    def actions_from_params(self, action_logits: torch.Tensor, deterministic: bool = False) -> torch.Tensor:
        # Update the proba distribution
        self.distribution(action_logits)
        return self.get_actions(deterministic=deterministic)

    def logp_from_params(self, action_logits: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        actions = self.actions_from_params(action_logits)
        log_prob = self.logp(actions)
...
class MultiCategoricalDistribution(Distribution):
    ...
    def prob_distribution(self, action_logits: torch.Tensor) -> "MultiCategoricalDistribution":
        self.distributions = [Categorical(logits=split) for split in torch.split(action_logits, tuple(self.action_dims), dim=1)]
        return self

    def logp(self, actions: torch.Tensor) -> torch.Tensor:
        # Extract each discrete action and compute log prob for their respective distributions
        return torch.stack(
            [dist.logp(action) for dist, action in zip(self.distributions, torch.unbind(actions, dim=1))], dim=1
        ).sum(dim=1)

    def entropy(self) -> torch.Tensor:
        return torch.stack([dist.entropy() for dist in self.distributions], dim=1).sum(dim=1)

    def sample(self) -> torch.Tensor:
        return torch.stack([dist.sample() for dist in self.distributions], dim=1)

    def mode(self) -> torch.Tensor:
        return torch.stack([torch.argmax(dist.probs, dim=1) for dist in self.distributions], dim=1)
...
class BernoulliDistribution(Distribution):
    ...
    def prob_distribution(self, action_logits: torch.Tensor) -> "BernoulliDistribution":
...
class StateDependentNoiseDistribution(Distribution):
    ...
    def actions_from_params(
        ...
        # Update the proba distribution
        self.distribution(mean_actions, log_std, latent_sde)
        return self.get_actions(deterministic=deterministic)

    def logp_from_params(
        self, mean_actions: torch.Tensor, log_std: torch.Tensor, latent_sde: torch.Tensor
