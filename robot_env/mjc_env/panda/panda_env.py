...
class PandaEnv(mujoco_env.MujocoEnv, utils.EzPickle):
    def __init__(self, frame_skip=5, mode="torque"):
        utils.EzPickle.__init__(**locals())
        module_path = os.path.dirname(__file__)
        if mode == "torque":
            model_path = "robots/franka-mujoco/franka_panda.xml"
        elif mode == "position":
            model_path = "robots/franka-mujoco/franka_panda_position.xml"
        else:
